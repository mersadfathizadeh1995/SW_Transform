"""Array configuration panel component.

Provides GUI for configuring geophone array parameters:
...
class ArrayConfigPanel(tk.Frame):
    """Collapsible panel for configuring geophone array parameters.
    
    Usage:
        panel = ArrayConfigPanel(parent)
    ...
    def _create_variables(self):
        ...
        self.source_position_var = tk.StringVar(value="-10.0")
        ...
        for var in [self.n_channels_use_var, self.channel_start_var, self.channel_end_var,
                    ...
                    self.source_position_var]:
            var.trace_add("write", self._on_var_change)
    
    def _build_ui(self):
        """Build the collapsible array config panel UI."""
        self._build_header()
        self._build_content()
    ...
    def _build_header(self):
        ...
        self.title_label = tk.Label(self.header, text="Array Configuration", 
    ...
    def _build_source_section(self):
        """Build source position section."""
        frame = tk.LabelFrame(self.content, text="Source Position")
        frame.pack(fill="x", padx=4, pady=4)
        
        row1 = tk.Frame(frame)
        row1.pack(fill="x", padx=4, pady=2)
        
        tk.Label(row1, text="Position (m):").pack(side="left")
        tk.Entry(row1, textvariable=self.source_position_var, width=8).pack(side="left", padx=2)
        tk.Label(row1, text="(negative = before array, positive = after)").pack(side="left", padx=8)
        
        row2 = tk.Frame(frame)
        row2.pack(fill="x", padx=4, pady=2)
        
        tk.Label(row2, text="Interior shot side:").pack(side="left")
        tk.Radiobutton(row2, text="Left", variable=self.interior_side_var, 
                      value="left").pack(side="left", padx=4)
        tk.Radiobutton(row2, text="Right", variable=self.interior_side_var,
                      value="right").pack(side="left", padx=4)
        tk.Radiobutton(row2, text="Both", variable=self.interior_side_var,
    ...
    def get_config(self) -> ArrayConfig:
        ...
        return ArrayConfig(
            ...
            source_position=float(self.source_position_var.get() or "-10.0"),
            interior_side=self.interior_side_var.get()
        )
    ...
    def set_config(self, config: ArrayConfig):
        ...
        self.source_position_var.set(f"{config.source_position:.1f}")
        self.interior_side_var.set(config.interior_side)
